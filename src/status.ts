...
function costOf(history: readonly PublishRecord[]): number {
...
async function main(): Promise<void> {
  ...
  const [runs, emails] = await Promise.all([
    fetchRuns(cron).catch((e: unknown) => {
      console.warn(`[status] GitHub unreachable: ${e instanceof Error ? e.message : e}`);
      return [] as Run[];
    }),
    (async () => {
      try {
        ...
        console.warn(`[status] Buttondown unreachable: ${e instanceof Error ? e.message : e}`);
        return [] as Email[];
      }
    })(),
  ]);
  ...
  const delays = scheduled.map((r) => r.delayMinutes).filter((d): d is number => d !== null);
  ...
  const gaps = cal.slice(0, -1).filter((d) => d.count === 0 && !d.before).length;
  ...
  console.log('[稼働]');
  line('最新の実行', lastRun ? `#${lastRun.number} ${jst(lastRun.createdAt)} JST — ${lastRun.conclusion ?? lastRun.status}` : '(取得できません)');
  line('直近30件の失敗', failures.length === 0 ? 'なし' : `${failures.length}件 (#${failures.map((f) => f.number).join(', #')})`);
  if (avgDelay !== null) {
    line('スケジュール遅延', `平均 ${Math.floor(avgDelay / 60)}時間${avgDelay % 60}分 / 最大 ${Math.max(...delays)}分`);
  }
  const next = nextRun(cron);
  line('次回予定', next ? `${jst(next)} JST (遅延見込み込みで +${avgDelay ?? 0}分)` : '(cron未検出)');

  console.log('\n[記事]');
  line('公開ページ', `${live}/${emails.length}`);
  line('下書きのまま', drafts.length === 0 ? 'なし' : `${drafts.length}件 — 公開ページなし`);
  line('リンク切れ', dead.length === 0 ? 'なし' : `${dead.length}件`);
  line('医療比率', `${Math.round(hcShare * 100)}% (直近10件 / 目標 ${Math.round(config.HEALTHCARE_RATIO * 100)}%)`);
  line('14日カレンダー', cal.map((d) => (d.before ? ' ' : d.count === 0 ? '·' : d.count > 1 ? '#' : '■')).join(''));
  if (gaps > 0) line('', `↑ ${gaps}日、記事が出ていません`);

  console.log('\n[コスト]');
  const cost = costOf(history);
  line('生成コスト累計', `$${cost.toFixed(2)} (${history.length}記事 / 平均 $${(cost / Math.max(1, history.length)).toFixed(3)})`);
  line('損益分岐', `有料会員 ${Math.ceil(900 / netAfterStripe(config.SUBSCRIPTION_PRICE_CENTS))}人 で Buttondownアドオン($9/月)を回収`);
  line('現在の月額費用', '$0 (有料アドオン未契約)');

  await mkdir(resolve(process.cwd(), 'data'), { recursive: true });
  await writeFile(
    OUT_PATH,
    renderHtml({ runs, emails, history, cal, cron, avgDelay, next, config }),
    'utf8',
  );
  console.log(`\nダッシュボード: ${OUT_PATH}\n`);
}
